def main():
    ...
    h1 = H1RobotCfg(
        ...
        sensors=[
            h1_camera_cfg.update(name='camera', resolution=(320, 240), pointcloud=True),
        ],
    )
    ...
    while env.simulation_app.is_running():
        i += 1
        action = move_action
        obs, _, terminated, _, _ = env.step(action=[{'h1': action}, {'h1': action}, {'h1': action}, {'h1': action}])

        if i % 500 == 0:
            print(f'============ {i} =============')
            assert_pointcloud(obs[0]['h1']['sensors'][h1_camera_cfg.name]['pointcloud'])
            assert_pointcloud(obs[1]['h1']['sensors'][h1_camera_cfg.name]['pointcloud'])
            assert_pointcloud(obs[2]['h1']['sensors'][h1_camera_cfg.name]['pointcloud'])
            assert_pointcloud(obs[3]['h1']['sensors'][h1_camera_cfg.name]['pointcloud'])
        if i == 2000:
